...
<script>
  export default {
      ...
      toforPath(url){
        if(url!=null){
         ...
         /* this.$router.push({path:'/user/user'})
          this.$router.push({path:'/role/juese'})
         this.$router.push({path:'/yewu/quanxian'})*/
        }
    ...
    mounted(){
     /* this.$axios.post(this.domain.ssoserverpath+"getRole").then((reponse)=>{
          //为data中的listMenu赋值
          this.listMenu=reponse.data;
